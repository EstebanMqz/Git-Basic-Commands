...
|[push](https://git-scm.com/docs/git-push)|<ol style="list-style-type:none;"><li>git push<li>git push -u `<origin branch>`<li>git push --set-upstream `<origin branch>`<li>git push --all<li>git push `<origin>` --delete `<branch>`<li> git push --force <li> git push --force-with-lease <li> git push --prune `<origin refs/heads/*>` <li> git push --mirror|<ol style="list-style-type:none;"><li>Push commits to upstream. <li>Push commits and set as upstream.<li>Push commits and set as upstream (too).<li>Pushes all  branches to upstream.<li>Delete a remote branch.<li>Push commits and destroy unmerged changes.<li>Push commits and only destroys personal unmerged changes.<li>Remove remote branches that don't have a local counterpart.<li> Overwrite remote with local branches, refs. and attributes.|
