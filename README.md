...
|[push](https://git-scm.com/docs/git-push)|<ol style="list-style-type:none;"><li>git push<li>git push -u `<origin branch>`<li>git push --set-upstream `<origin branch>`<li>git push --all<li>git push `<origin>` --delete `<branch>`<li> git push --force <li> git push --force-with-lease <li> git push --prune `<origin refs/heads/*>` <li> git push --mirror|<ol style="list-style-type:none;"><li>Push commits to upstream. <li>Push commits and set as upstream.<li>Push commits and set it as upstream too.<li>Pushes all the branches to upstream.<li>Delete a remote branch.<li>Push commits and destroy unmerged changes.<li>Push commits and destroys personal unmerged changes.<li>Remove remote branches without local counterpart.<li> Overwrite remote with local branches, refs and attributes.|
|[fetch](https://git-scm.com/docs/git-fetch)|<ol style="list-style-type:none;"><li>git fetch `<origin>`<li>git fetch -u `<origin branch>`<li>git fetch --all<li>git fetch --dry-run<li>git fetch --append<li>git fetch --depth=`<depth>`|<ol style="list-style-type:none;"><li>Fetch all branches from the repository.<li>Fetch the specified branch in the repository.<li>Fetches all the branches from the upstream.<li>Show output without executing the fetch.<li> Without this option, old data in .git/FETCH_HEAD would be overwritten.<li> Limit fetching to a commit number from the tip.|
|[diff](https://git-scm.com/docs/git-diff)|<ol style="list-style-type:none;"><li>git diff<li>git diff --staged<li> git diff HEAD<li> git diff --color-words<li>git commit `<ID 1> <ID 2>`|<ol style="list-style-type:none;"><li>Check for differences between fetched local and upstream branches.<li> Check for differences between staged and local.<li>Check for differences between working dir. and last commit in remote.<li> Highlight changes with a better color granularity.<li> Check for differences between two commits in a repository.|
|[config](https://git-scm.com/docs/git-config)|<ol style="list-style-type:none;"><li>git config --global --edit<li>git config --global user.name `<username>`<li> git config --global user.email `<e-mail>`<li>git config --system user.name `<organization>`<li> git config --get user.name<li>git config remote.origin.prune true<li>git config --global fetch.prune true|<ol style="list-style-type:none;"><li>Display global config for .git file. Create it if none is found.<li>Sets author name to commits, accesible to all users and projects.<li> Sets author email to commits, accesible to all users and projects.<li>Set author name for all git users in system config.<li> Show author or email (`user.email`, `--system user.name` or `--system user.email`).<li>Set automatic prune with git fetch/pull (to clean inactive remote-tracking branches).<li>Set automatic prune only with git fetch.|
|[log](https://git-scm.com/docs/git-log)|<ol style="list-style-type:none;"><li>git log --pretty=oneline<li>|<ol style="list-style-type:none;"><li>Show commits and IDs for current repository.|
